...
- 추상클래스 : 자바에 존재하는 추상클래스와 같음. 추상클래스 안에는 개발자가 상속받아 구현해야 하는 미구현 메소드가 존재하기 때문에 그 자체로 인스턴스화 될 수 는 없고, 반드시 온전한 클래스에서 추상클래스를 상속받아 구현을 완료 해야 함
- 트레이트와 추상클래스의 공통점 : 그 자체로 인스턴스화 불가능.
...
- 인터페이스를 쓰지 않고 추상 클래스를 사용한다면 : 
- 추상클래스는 다중 상속이 되지 않아 원천적으로 여러 특성의 클래스를 도입하는 데 한계가 있음.

### 셀프 타입 self_type

### 트레이트를 이용하여 인스턴스화

- 트레이트를 사용하는 다른 방식으로는 **클래스가 인스턴스화 될 때 클래스에 트레이트를 추가하는 것**
