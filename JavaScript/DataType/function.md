...
"실행가능한 코드가 포함된 특수한 타입의 객체"라고 보는 것이 이해하기 편하다. 

그리고 (화살표 함수를 제외한) 모든 함수는 다른 객체와는 달리 prototype이라는 프로퍼티를 가진다.
...
그 중에서도 prototype으로 객체지향을 구현한 언어인데, 이러한 특성에 따라 자바스크립트에서의 모든 객체는 함수로 만들어진다. 
...
(화살표 함수와 제너레이터 함수는 prototype 에 constructor를 갖고 있지 않다. )
