...
- 자바스크립트는 싱글스레드이다. 차선이 하나밖에 없다는 말이다. 



#### 콜스택에는 얼마만큼의 덩어리가 올라가는가?



#### 얼마나 오랫동안 붙들어야 블로킹인가?

- 엄밀히 말하면 모든 프로그램은 Blocking 이다. 실행되는 순간에는 끼어들 수가 없기 때문이다. 
- Non blocking  과 blocking을 가르는 기준은 "시간"이다. 
- Sync Flow 가 납득할만한 시간 내에 종료된다면, 이를 Non Blocking으로 판단해준다. 
- 웹에서의 Blocking 의 시간 제약 : 5초?? 그 근거는?
...
- 만약에 콘솔점로그가 한개 찍히는데 10초라고 하면, 이건 블로킹일 것이다. 
- 하지만 이게 0.00000001초만에 끝나버린다면?
