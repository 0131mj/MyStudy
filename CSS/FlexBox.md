...
- flex-direction에서 **여백**을 차지하는 비중을 설정한다. 
...
- flex방향에서 차지할 크기를 지정해주는 옵션
- auto : 기본 자기 값,
- 0으로 설정하고 flex-grow를 설정하면, 기본값이 없이 flex-grow에만 참조해서 가져운다. 기본으로  0으로 세팅하는 경우가 많음
...
- 해당영역이 줄어들때 고통분담을 얼마나 할거냐
- 0일경우 고통분담읗 하지 않음
- 기본적으로 basis 값을 가지고 있어야 사용이 가능하다



### flex

- 그냥 flex키워드로  숫자만 쓰는 경우
- flex grow 를 뜻하지만, 동시에 flex-basis가 0으로 자동으로 세팅된다. 